(() => {
  ...
  function renderLogin() {
    ...
    app.innerHTML = `
      <div class="login-page">
        <div class="login-card simple-login-card">
          <div class="brand"><div class="brand-mark">ES</div><div><h1>Reportform ZM</h1><p>Enter results once. Send reports to parents. — V2.9</p></div></div>
          ...
          <form id="loginForm" class="simple-login-form">
            ...
            <div class="field"><label>PIN / password</label><input id="password" type="password" autocomplete="current-password" placeholder="Enter PIN or password" required></div>
            <button class="btn btn-primary full btn-lg" type="submit">Continue</button>
          </form>
          <button id="createTeacherAccount" class="btn btn-secondary full create-account-btn" type="button">Create teacher account</button>
          <div class="login-help-text">New teacher? Create your account once. The school administrator approves it before you can enter results.</div>
    ...
    byId('createTeacherAccount').onclick=renderRegister;
  ...
  async function renderRegister() {
    disconnectEvents();
    let opts={departments:[]}; try{opts=await api('/api/register/options')}catch{}
    app.innerHTML=`<div class="login-page"><div class="login-card simple-login-card">
      <div class="brand"><div class="brand-mark">ES</div><div><h1>Create teacher account</h1><p>${esc(opts.schoolName||'Reportform ZM')}</p></div></div>
      <div class="login-hero"><b>Teacher registration</b><span>Create your login. Administration verifies your account before any school results become accessible.</span></div>
      <form id="registerForm" class="simple-login-form">
        <div class="field"><label>Full name</label><input id="regName" autocomplete="name" placeholder="e.g. Pearson Kamanga" required></div>
        <div class="field"><label>Staff ID / username</label><input id="regUsername" autocomplete="username" autocapitalize="none" placeholder="e.g. pkamanga" required></div>
        <div class="field"><label>Phone number</label><input id="regPhone" inputmode="tel" autocomplete="tel" placeholder="School contact number"></div>
        <div class="field"><label>Department</label><select id="regDepartment"><option value="">Not sure / select later</option>${(opts.departments||[]).map(d=>`<option value="${d.id}">${esc(d.name)}</option>`).join('')}</select></div>
        <div class="field"><label>Create PIN / password</label><input id="regPassword" type="password" autocomplete="new-password" placeholder="At least 6 characters" required></div>
        <div class="field"><label>Confirm PIN / password</label><input id="regConfirm" type="password" autocomplete="new-password" placeholder="Enter it again" required></div>
        <button class="btn btn-primary full btn-lg" type="submit">Create account</button>
      </form>
      <button id="backToLogin" class="btn btn-secondary full create-account-btn" type="button">Back to sign in</button>
      <div class="login-help-text">After approval, sign in and EduSend will ask which subjects and classes you teach.</div>
    </div></div>`;
    byId('backToLogin').onclick=renderLogin;
    byId('registerForm').onsubmit=async e=>{
      e.preventDefault();
      if(byId('regPassword').value!==byId('regConfirm').value){toast('PIN/passwords do not match',true);return}
      const btn=e.submitter||e.target.querySelector('button[type="submit"]');btn.disabled=true;btn.textContent='Sending request…';
      try{
        const r=await api('/api/register',{method:'POST',body:{name:byId('regName').value,username:byId('regUsername').value,phone:byId('regPhone').value,departmentId:byId('regDepartment').value||null,password:byId('regPassword').value}});
        actionPopup('Account request sent',r.message||'Administration will approve your account.');
        renderLogin();
      }catch(err){toast(err.message,true);btn.disabled=false;btn.textContent='Create account'}
    };
  }
  ...
  function renderShell() {
    ...
    app.innerHTML = `
      <div class="shell">
        <aside class="sidebar">
          <div class="side-brand"><div class="brand-mark">ES</div><div><strong>EduSend</strong><div class="tiny">School Results V2.9</div></div></div>
  ...
  async function renderDashboard(content) {
    ...
    content.innerHTML = `
      <section class="hero-card simple-hero"><div><span class="eyebrow">REPORTFORM ZM • EDUSEND V2.9</span><h1>Welcome, ${esc(firstName)}</h1><p>Your main job is simple: enter results once, prepare the report, send it to the parent.</p></div><div class="hero-orb">ES</div></section>
  ...
  async function renderAdmin(content) {
    ...
    ${(d.pendingRegistrations||[]).length?`<div class="card account-requests-card"><div class="section-title"><div><span class="eyebrow">NEW TEACHER ACCOUNTS</span><h3 style="margin-top:4px">${d.pendingRegistrations.length} account request${d.pendingRegistrations.length===1?'':'s'} waiting</h3></div><span class="pill pill-orange">Approval required</span></div><p class="small muted">Check the staff member before approving. Approval creates a normal teacher account; subject and class access are still verified separately.</p>${d.pendingRegistrations.map(r=>{const dep=d.departments.find(x=>x.id===r.departmentId);return `<div class="account-request-row"><div><b>${esc(r.name)}</b><div class="tiny muted">Staff ID: ${esc(r.username)}${r.phone?` • ${esc(r.phone)}`:''}${dep?` • ${esc(dep.name)}`:''}</div></div><div class="inline"><button class="btn btn-green" data-reg-approve="${r.id}">Approve</button><button class="btn btn-danger" data-reg-decline="${r.id}">Decline</button></div></div>`}).join('')}</div>`:''}
    ...
    content.querySelectorAll('[data-reg-approve]').forEach(btn=>btn.onclick=async()=>{
      btn.disabled=true;
      try{const r=await api('/api/admin/registration-decide',{method:'POST',body:{userId:btn.dataset.regApprove,approve:true}});actionPopup('Teacher account approved',`${r.user.name} can now sign in. EduSend will ask them to set up their teaching profile.`);await renderAdmin(content)}catch(err){toast(err.message,true);btn.disabled=false}
    });
    content.querySelectorAll('[data-reg-decline]').forEach(btn=>btn.onclick=async()=>{
      if(!confirm('Decline this teacher account request?'))return;btn.disabled=true;
      try{await api('/api/admin/registration-decide',{method:'POST',body:{userId:btn.dataset.regDecline,approve:false}});toast('Account request declined');await renderAdmin(content)}catch(err){toast(err.message,true);btn.disabled=false}
    });
