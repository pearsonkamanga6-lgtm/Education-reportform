...
function migrateData(raw) {
  ...
  db.users.forEach(u => { if (u.phone === undefined) u.phone = ''; if (u.active === undefined) u.active = true; if (!u.registrationStatus) u.registrationStatus = u.active === false ? 'PENDING' : 'APPROVED'; u.profileSetupComplete = !!u.profileSetupComplete || db.teachingAssignments.some(a=>a.teacherUserId===u.id); });
...
async function api(req, res, urlObj) {
  ...
  // Public teacher self-registration. New accounts remain inactive until an administrator approves them.
  if (req.method === 'GET' && pathname === '/api/register/options') {
    return sendJson(res, 200, {
      schoolName: db.school?.name || 'Reportform ZM School',
      departments: db.departments.filter(d => d.active !== false).map(d => ({ id:d.id, name:d.name }))
    });
  }
  ...
  if (req.method === 'POST' && pathname === '/api/register') {
    ...
    const username = String(body.username || body.staffId || '').trim().toLowerCase();
    const phone = String(body.phone || '').trim().slice(0, 40);
    const password = String(body.password || body.pin || '');
    const departmentId = body.departmentId && db.departments.some(d => d.id === body.departmentId) ? body.departmentId : null;
    if (name.length < 3) return sendError(res, 400, 'Enter your full name');
    if (!/^[a-z0-9._-]{3,40}$/.test(username)) return sendError(res, 400, 'Staff ID must be 3–40 characters using letters, numbers, dot, dash or underscore');
    if (password.length < 6) return sendError(res, 400, 'Create a PIN/password with at least 6 characters');
    if (db.users.some(u => String(u.username || '').toLowerCase() === username)) return sendError(res, 409, 'That staff ID already has an account');
    const duplicatePending = db.users.find(u => String(u.name || '').toLowerCase() === name.toLowerCase() && u.registrationStatus === 'PENDING');
    if (duplicatePending) return sendError(res, 409, 'An account request for this staff member is already waiting for approval');
    const newUser = {
      id:id('usr'), name, username, passwordHash:hashPassword(password), roles:['TEACHER'],
      departmentId, departmentIds:departmentId ? [departmentId] : [], phone,
      active:false, registrationStatus:'PENDING', registrationRequestedAt:nowIso(),
      registrationDecidedAt:null, registrationDecidedByUserId:null, profileSetupComplete:false
    };
    db.users.push(newUser);
    const admins = db.users.filter(u => u.active !== false && (hasRole(u,'ADMIN') || hasRole(u,'HEAD'))).map(u => u.id);
    createNotification(admins, 'TEACHER_ACCOUNT_REQUEST', 'New teacher account request', `${name} requested a teacher account${username ? ` (${username})` : ''}.`, { userId:newUser.id });
    audit(null, 'TEACHER_ACCOUNT_REQUESTED', `${name} (${username})`);
    saveData();
    return sendJson(res, 201, { ok:true, status:'PENDING', message:'Account request sent to the school administrator for approval.' });
  }
  ...
  if (req.method === 'GET' && pathname === '/api/admin/setup') {
    if (!hasRole(user, 'ADMIN')) return sendError(res, 403, 'Administrator access required');
    return sendJson(res, 200, { school: db.school, users: db.users.map(safeUser), departments: db.departments, classes: db.classes, subjects: db.subjects, assessments: db.assessments, teachingAssignments: db.teachingAssignments.map(assignmentView), pendingRegistrations: db.users.filter(u=>u.registrationStatus==='PENDING').map(safeUser) });
  }

  if (req.method === 'POST' && pathname === '/api/admin/registration-decide') {
    if (!hasRole(user, 'ADMIN') && !hasRole(user, 'HEAD')) return sendError(res, 403, 'Administration access required');
    const body = await readJson(req);
    const candidate = db.users.find(u => u.id === body.userId && u.registrationStatus === 'PENDING');
    if (!candidate) return sendError(res, 404, 'Pending account request not found');
    const approve = !!body.approve;
    candidate.registrationStatus = approve ? 'APPROVED' : 'DECLINED';
    candidate.registrationDecidedAt = nowIso();
    candidate.registrationDecidedByUserId = user.id;
    candidate.active = approve;
    if (approve) {
      candidate.roles = ['TEACHER'];
      candidate.profileSetupComplete = false;
      audit(user.id, 'TEACHER_ACCOUNT_APPROVED', `${candidate.name} (${candidate.username})`);
    } else {
      audit(user.id, 'TEACHER_ACCOUNT_DECLINED', `${candidate.name} (${candidate.username})`);
    }
    saveData();
    return sendJson(res, 200, { ok:true, approved:approve, user:safeUser(candidate) });
  }
  ...
  if (req.method === 'POST' && pathname === '/api/admin/user') {
    ...
    const newUser = { id: id('usr'), name, username, passwordHash: hashPassword(password), roles: roles.length ? roles : ['TEACHER'], departmentId: body.departmentId || null, departmentIds: body.departmentId ? [body.departmentId] : [], phone: String(body.phone || '').trim(), active: true, registrationStatus:'APPROVED', registrationRequestedAt:null, registrationDecidedAt:nowIso(), registrationDecidedByUserId:user.id, profileSetupComplete:false };
    db.users.push(newUser);
